...
This page presents exploratory analysis of the 2024 IPUMS employment dataset and Lightcast IT job postings, contributed by each team member. Julie examinesthe gender wage gap by industry and state and links job market activity to wages across states. Antara compares gender representation and wages between the IT workforce and the overall workforce. Yifan analyzes employment rates and income distributions by age and gender. Zhiyang explores industry concentration and geographic household distribution. Alina examines salary transparency and industry distribution in the Lightcast and IPUMS datasets.
...
employment_rate_age = pd.read_csv(
...
fig = px.bar(
    ...
    color="SEX_LABEL",
    barmode="group",
    text=employment_rate_age["Employment_Rate"].round(1),
...
```{python}
household = pd.read_parquet("../data/processed/employed_only.parquet")
